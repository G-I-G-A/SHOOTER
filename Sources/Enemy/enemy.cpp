...
Enemy::Enemy(std::string name, float health, float power, float speed, float posX, float posY)
    : m_name(name), m_health(health), m_power(power), m_speed(speed), m_posX(posX), m_posY(posY)
{
    EnemyDraw();
...
void Enemy::EnemyDraw(){
    ...
    this->m_textureEnemy.loadFromFile("C:/Qt/Projects/TOWERDEF/Assets/Sprites/Enemy.png");
...
void Enemy::setPositionEnemy(float x, float y){
    ...
    this->enemySprite.setPosition(m_posX, m_posY);
